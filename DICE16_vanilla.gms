...
PARAMETERS
** Availability of fossil fuels - 1 ton of C = 3.6 ton of C02
        fosslim  Maximum cumulative extraction fossil fuels (GtC)  /6000/
**Time Step
        tstep    Years per Period                                  /5/
** If optimal control - base (0 no feedback of climate change - 1 climate change give problems)
        ifopt    Indicator where optimized is 1 and base is 0      /1/
** Preferences
        elasmu   Elasticity of marginal utility of consumption     /1.45 /
*discounting - very important in cost benefit analysis, not so much in cost effective
...
* assumption that deforestation will go to zero over time
...
** Climate damage parameters
        ...
        a2        Damage quadratic term                            /0.00116 /
        a3        Damage exponent                                  /3.00    /
** Abatement cost
...
*        TCONST(t)        Put temperature constraint on the model
...
If (ifopt eq 0,
*a2 is the exponent of the damage function -> if 0 there's no climate change
       a2 = 0;
...
*=== First unload to GDX file for selected variables
execute_unload "results.gdx", TATM.L,Y.L,DAMFRAC.L,CPC.L,cprice.L,MIU.L,scc,RI.l,E.l;
*export to xls
execute '=gdx2xls results.gdx';
execute '=shellExecute results.xlsx';


*also create a gdx with all results
execute_unload "results_all.gdx"
*export to xls
execute '=gdx2xls results_all.gdx';
